...
/**
 * Every environment variable that can lead the library to a real itch install.
 *
 * `HOME`/`USERPROFILE` back `os.homedir()`, the rest are read directly by
 * `getItchPathCandidates()`.
 */
const ITCH_LOOKUP_VARS = [
  "HOME",
  "USERPROFILE",
  "XDG_CONFIG_HOME",
  "APPDATA",
...
/**
 * Runs `fn` with the environment pointed at an empty home directory.
 *
 * Without this, any test that calls `findItchPath()` without an explicit path
 * reaches whatever itch install the developer happens to have, so it passes
 * for the wrong reason on one machine and the right reason on another.
 * `overrides` are applied after the environment is cleared.
 */
export async function withIsolatedHome(overrides, fn) {
  const saved = new Map();
  for (const key of [...ITCH_LOOKUP_VARS, ...Object.keys(overrides)]) {
    saved.set(key, process.env[key]);
    delete process.env[key];
  }

  const home = fs.mkdtempSync(path.join(os.tmpdir(), `find-itch-home-${counter++}-`));
