{#-
   find-itch-games markdown reference.

   Derived from shellman's wikipage.md template.
   ...
   Changed: option descriptions and function cross-references are passed
   through the link() macro, so each command links to the function that
   implements it. shellman does not do this natively.
-#}
...
- {% if option.short %}**`{{ option.short }}`**{% if option.long %},{% endif %} {% endif %}{% if option.long %}**`{{ option.long }}`**{% if option.positional %} {% endif %}{% endif %}{% if option.positional %}*`{{ option.positional }}`*{% endif %}:
  {{ link(option.description|e)|indent(2) }}
{% endfor %}
{% if not loop.last %}{{ '\n' }}{% endif %}
{% endfor %}
...
{% include "linked_function.md" with context %}
{% if not loop.last %}
---
{% endif %}
{% endfor %}
