...
describe("shared definitions", () => {
  // These assert against shared/definitions.json, which the Python suite reads
  // too, so the implementations cannot disagree about the vocabulary.
  it("matches the shared strategy list", () => {
    assert.deepEqual([...ITCH_STRATEGIES], DEFINITIONS.strategies.values);
  });

  it("matches the shared app names", () => {
    assert.deepEqual([...ITCH_APP_NAMES], DEFINITIONS.appNames.values);
  });

  it("matches the shared receipt layout", () => {
    const installPath = path.join("/games", "some-game");
    assert.equal(RECEIPT_DIR, DEFINITIONS.receipt.directory);
    assert.equal(
      getReceiptPath(installPath),
      path.join(installPath, DEFINITIONS.receipt.directory, DEFINITIONS.receipt.fileName),
    );
    assert.equal(
      getLegacyReceiptPath(installPath),
      path.join(installPath, DEFINITIONS.receipt.directory, DEFINITIONS.receipt.legacyFileName),
    );
  });

  it("matches the shared staging folder and appdata location", () => {
    assert.equal(
      getLibraryDownloadsFolder("/games"),
      path.join("/games", DEFINITIONS.stagingFolderName.value),
    );
    assert.equal(
      getAppdataInstallLocation("/itch"),
      path.join("/itch", DEFINITIONS.builtinInstallLocationId.relativePath),
    );
  });

  it("matches the shared database file name", () => {
    assert.equal(
      getDatabasePath("/itch"),
      path.join("/itch", "db", DEFINITIONS.databaseFileName.value),
    );
  });
});
