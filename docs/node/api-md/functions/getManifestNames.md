[find-itch-games](../README.md) / getManifestNames

# Function: getManifestNames()

```ts
function getManifestNames(manifest): string[];
```

Defined in: [src/manifest.ts:95](https://github.com/sparr/find-itch-games/blob/main/node/src/manifest.ts#L95)

Names a manifest answers to in [findItchAppByName](findItchAppByName.md).

## Parameters

### manifest

[`IItchAppManifest`](../interfaces/IItchAppManifest.md)

The game to list names for.

## Returns

`string`[]
